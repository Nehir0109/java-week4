...
public class Main{
    public static void main(String[] args) {
        ...
        int[] array= {1,2,3,4,4,4,4,4,5,6,7,7,8,9,10};
        ...
        System.out.print("Ortalama= "+ (sum/ array.length));
        ...
        for(int i=0; i<array.length; i++){
            for(int j=0; j<array.length; j++){
                if(array[i]==array[j]){ //tekrar eden sayı varlığını kontrol ettik
                    fr++;
                }
            }
            if(fr>mode){
        ...
        System.out.printf("\nDizinin modu(en sık görülen değer)= %d",mode);
